...
public class ComparecimentoService {
    ...
    /**
     * Registrar comparecimento - aceita processoId (preferencial) ou custodiadoId (deprecated)
     */
    @Transactional
    public HistoricoComparecimento registrarComparecimento(ComparecimentoDTO dto) {
        ...
        // Resolver processo e custodiado
        if (dto.getProcessoId() != null) {
            processo = processoRepository.findByIdComCustodiado(dto.getProcessoId())
                    .orElseThrow(() -> new EntityNotFoundException("Processo não encontrado com ID: " + dto.getProcessoId()));
            custodiado = processo.getCustodiado();
            log.info("Registrando comparecimento via processoId: {}", dto.getProcessoId());
        } else if (dto.getCustodiadoId() != null) {
            // Fallback: buscar por custodiadoId (deprecated)
            ...
            // Tentar encontrar processo ativo do custodiado
            ...
            log.info("Registrando comparecimento via custodiadoId (deprecated): {}", dto.getCustodiadoId());
        } else {
        ...
        HistoricoComparecimento historico = HistoricoComparecimento.builder()
                .processo(processo)
                .custodiado(custodiado) // mantido temporariamente
        ...
        // Processar mudança de endereço (endereço pertence ao custodiado)
        ...
        // Atualizar dados do processo (se existe) e custodiado
        if (dto.getTipoValidacao() != TipoValidacao.CADASTRO_INICIAL) {
            ...
            // Manter atualização no custodiado durante transição
        ...
        log.info("Comparecimento registrado - ID: {}, Processo: {}, Custodiado: {}",
                historicoSalvo.getId(),
                processo != null ? processo.getNumeroProcesso() : "N/A",
                custodiado.getNome());
    ...
    public List<HistoricoComparecimento> buscarComparecimentosPorPeriodo(LocalDate inicio, LocalDate fim) {
        if (inicio == null || fim == null) throw new IllegalArgumentException("Data de início e fim são obrigatórias");
        if (inicio.isAfter(fim)) throw new IllegalArgumentException("Data de início não pode ser posterior à data de fim");
    ...
    public Page<HistoricoComparecimentoResponseDTO> buscarComparecimentosComFiltros(
        ...
        return historicoComparecimentoRepository.findComFiltros(dataInicio, dataFim, tipoValidacao, pageable).map(this::toResponseDTO);
    }
    ...
    public HistoricoComparecimento atualizarObservacoes(Long historicoId, String observacoes) {
        HistoricoComparecimento historico = historicoComparecimentoRepository.findById(historicoId)
                .orElseThrow(() -> new EntityNotFoundException("Histórico não encontrado com ID: " + historicoId));
    ...
    public Map<String, Object> buscarEstatisticasDetalhadas() {
        ...
        double percentualConformidade = totalCustodiados > 0 ? (double) custodiadosEmConformidade / totalCustodiados * 100 : 0.0;
        double percentualInadimplencia = totalCustodiados > 0 ? (double) custodiadosInadimplentes / totalCustodiados * 100 : 0.0;
        ...
        estatisticas.put("percentualPresencial", totalComparecimentos > 0 ? (double) presenciais / totalComparecimentos * 100 : 0.0);
        estatisticas.put("percentualOnline", totalComparecimentos > 0 ? (double) online / totalComparecimentos * 100 : 0.0);
        estatisticas.put("totalMudancasEndereco", totalMudancasEndereco);
        estatisticas.put("comparecimentosHoje", custodiadoRepository.findComparecimentosHoje().size());
    ...
    public EstatisticasComparecimento buscarEstatisticas(LocalDate inicio, LocalDate fim) {
        List<HistoricoComparecimento> comparecimentos = historicoComparecimentoRepository.findByDataComparecimentoBetween(inicio, fim);
        long total = comparecimentos.size();

        return EstatisticasComparecimento.builder()
                .periodo(inicio + " a " + fim)
                .totalComparecimentos(total)
                .comparecimentosPresenciais(comparecimentos.stream().filter(h -> h.getTipoValidacao() == TipoValidacao.PRESENCIAL).count())
                .comparecimentosOnline(comparecimentos.stream().filter(h -> h.getTipoValidacao() == TipoValidacao.ONLINE).count())
                .cadastrosIniciais(comparecimentos.stream().filter(h -> h.getTipoValidacao() == TipoValidacao.CADASTRO_INICIAL).count())
                .mudancasEndereco(comparecimentos.stream().filter(HistoricoComparecimento::houveMudancaEndereco).count())
                .percentualPresencial(total > 0 ? (double) comparecimentos.stream().filter(h -> h.getTipoValidacao() == TipoValidacao.PRESENCIAL).count() / total * 100 : 0.0)
                .percentualOnline(total > 0 ? (double) comparecimentos.stream().filter(h -> h.getTipoValidacao() == TipoValidacao.ONLINE).count() / total * 100 : 0.0)
                .build();
    }

    @Transactional(readOnly = true)
    public EstatisticasGerais buscarEstatisticasGerais() {
        List<HistoricoComparecimento> todos = historicoComparecimentoRepository.findAll();
        long total = todos.size();
        long presenciais = todos.stream().filter(h -> h.getTipoValidacao() == TipoValidacao.PRESENCIAL).count();
        long online = todos.stream().filter(h -> h.getTipoValidacao() == TipoValidacao.ONLINE).count();
        long cadastros = todos.stream().filter(h -> h.getTipoValidacao() == TipoValidacao.CADASTRO_INICIAL).count();
        long mudancas = todos.stream().filter(HistoricoComparecimento::houveMudancaEndereco).count();
        long hoje = historicoComparecimentoRepository.findByDataComparecimento(LocalDate.now()).size();
        long esteMes = historicoComparecimentoRepository.findByDataComparecimentoBetween(LocalDate.now().withDayOfMonth(1), LocalDate.now()).size();
        long custodiadosDistintos = todos.stream().map(h -> h.getCustodiadoId()).filter(Objects::nonNull).distinct().count();

        return EstatisticasGerais.builder()
                .totalComparecimentos(total).comparecimentosPresenciais(presenciais)
                .comparecimentosOnline(online).cadastrosIniciais(cadastros)
                .totalMudancasEndereco(mudancas).comparecimentosHoje(hoje)
                .comparecimentosEsteMes(esteMes).custodiadosComComparecimento(custodiadosDistintos)
                .percentualPresencial(total > 0 ? (double) presenciais / total * 100 : 0.0)
                .percentualOnline(total > 0 ? (double) online / total * 100 : 0.0)
                .mediaComparecimentosPorCustodiado(custodiadosDistintos > 0 ? (double) total / custodiadosDistintos : 0.0)
                .build();
    }
    ...
    public ResumoSistema buscarResumoSistema() {
        ...
        return ResumoSistema.builder()
                ...
                .comparecimentosHoje(custodiadoRepository.findComparecimentosHoje().size())
                .totalComparecimentos(historicoComparecimentoRepository.count())
                .comparecimentosEsteMes(historicoComparecimentoRepository.findByDataComparecimentoBetween(hoje.withDayOfMonth(1), hoje).size())
                ...
                .percentualConformidade(totalCustodiados > 0 ? (double) custodiadosEmConformidade / totalCustodiados * 100 : 0.0)
                .percentualInadimplencia(totalCustodiados > 0 ? (double) custodiadosInadimplentes / totalCustodiados * 100 : 0.0)
                .dataConsulta(hoje)
    ...
    public Map<String, Object> migrarCadastrosIniciais(String validadoPor) {
        ...
        if (todosCustodiados.isEmpty()) return Map.of("status", "warning", "message", "Nenhum custodiado encontrado", "totalCustodiados", 0, "custodiadosMigrados", 0);
        ...
        for (Custodiado custodiado : todosCustodiados) {
            try {
                if (!historicoComparecimentoRepository.existsCadastroInicialPorCustodiado(custodiado.getId())) {
                    HistoricoComparecimento cadastroInicial = HistoricoComparecimento.builder()
                            ...
                            .validadoPor(validadoPor).observacoes("Cadastro inicial migrado").mudancaEndereco(Boolean.FALSE)
                            .build();
                    // Vincular ao primeiro processo se existir
                    List<Processo> processos = processoRepository.findProcessosAtivosByCustodiado(custodiado.getId());
                    if (!processos.isEmpty()) cadastroInicial.setProcesso(processos.get(0));
                    historicoComparecimentoRepository.save(cadastroInicial);
                    custodiadosMigrados++;
                }
            } catch (Exception e) {
                erros.add(String.format("Erro custodiado %s (ID: %d): %s", custodiado.getNome(), custodiado.getId(), e.getMessage()));
                log.error("Erro ao migrar custodiado {}: {}", custodiado.getId(), e.getMessage());
            }
        }

        return Map.of("status", "success",
                "message", String.format("Migração concluída. %d de %d migrados", custodiadosMigrados, todosCustodiados.size()),
                "totalCustodiados", todosCustodiados.size(), "custodiadosMigrados", custodiadosMigrados,
                "erros", erros.size(), "detalhesErros", erros);
    }
    ...
    private HistoricoComparecimentoResponseDTO toResponseDTO(HistoricoComparecimento h) {
        return HistoricoComparecimentoResponseDTO.builder()
                .id(h.getId()).custodiadoId(h.getCustodiadoId()).custodiadoNome(h.getCustodiadoNome())
                .dataComparecimento(h.getDataComparecimento()).horaComparecimento(h.getHoraComparecimento())
                .tipoValidacao(h.getTipoValidacao().name()).validadoPor(h.getValidadoPor())
                .observacoes(h.getObservacoes()).mudancaEndereco(h.getMudancaEndereco())
                .motivoMudancaEndereco(h.getMotivoMudancaEndereco()).build();
    }
    ...
    private void limparEFormatarDadosDTO(ComparecimentoDTO dto) {
        ...
        if (dto.houveMudancaEndereco() && dto.getNovoEndereco() != null) {
            ...
            if (end.getCep() != null) {
                ...
                if (cepLimpo.length() == 8) end.setCep(cepLimpo.substring(0, 5) + "-" + cepLimpo.substring(5));
            }
    ...
    private void validarComparecimento(ComparecimentoDTO dto) {
        if (dto.getProcessoId() == null && dto.getCustodiadoId() == null)
            throw new IllegalArgumentException("processoId ou custodiadoId é obrigatório");
        if (dto.getDataComparecimento() == null) throw new IllegalArgumentException("Data do comparecimento é obrigatória");
        if (dto.getDataComparecimento().isAfter(LocalDate.now())) throw new IllegalArgumentException("Data não pode ser futura");
        if (dto.getTipoValidacao() == null) throw new IllegalArgumentException("Tipo de validação é obrigatório");
        if (dto.getValidadoPor() == null || dto.getValidadoPor().trim().isEmpty()) throw new IllegalArgumentException("Validado por é obrigatório");
        if (dto.houveMudancaEndereco()) {
            if (dto.getNovoEndereco() == null) throw new IllegalArgumentException("Dados do novo endereço obrigatórios quando há mudança");
            if (!dto.getNovoEndereco().isCompleto()) throw new IllegalArgumentException("Preencha todos os campos obrigatórios do endereço");
        }
    }
    ...
    private RelatorioUltimosMeses calcularRelatorioUltimosMeses(int meses) {
        ...
        List<HistoricoComparecimento> comparecimentos = historicoComparecimentoRepository.findByDataComparecimentoBetween(inicio, hoje);
        long total = comparecimentos.size();
        long presenciais = comparecimentos.stream().filter(h -> h.getTipoValidacao() == TipoValidacao.PRESENCIAL).count();
        long online = comparecimentos.stream().filter(h -> h.getTipoValidacao() == TipoValidacao.ONLINE).count();
        long mudancas = comparecimentos.stream().filter(HistoricoComparecimento::houveMudancaEndereco).count();

        return RelatorioUltimosMeses.builder().mesesAnalisados(meses).periodoInicio(inicio).periodoFim(hoje)
                .totalComparecimentos(total).comparecimentosPresenciais(presenciais).comparecimentosOnline(online)
                .mudancasEndereco(mudancas).mediaComparecimentosMensal((double) total / meses)
                .percentualPresencial(total > 0 ? (double) presenciais / total * 100 : 0.0)
                .percentualOnline(total > 0 ? (double) online / total * 100 : 0.0).build();
    }
    ...
    private List<TendenciaMensal> calcularTendenciaConformidade(int meses) {
        ...
        for (int i = 0; i < meses; i++) {
            ...
            List<HistoricoComparecimento> comp = historicoComparecimentoRepository.findByDataComparecimentoBetween(inicioMes, fimMes);
            long custodiadosComp = comp.stream().map(HistoricoComparecimento::getCustodiadoId).filter(Objects::nonNull).distinct().count();
            long inadimplentes = totalCustodiados - custodiadosComp;
            double taxaConf = totalCustodiados > 0 ? (double) custodiadosComp / totalCustodiados * 100 : 0.0;
            String mesNome = ref.format(DateTimeFormatter.ofPattern("MMMM/yyyy", Locale.forLanguageTag("pt-BR")));

            tendencias.add(TendenciaMensal.builder()
                    .mes(ref.format(DateTimeFormatter.ofPattern("yyyy-MM"))).mesNome(mesNome)
                    .totalCustodiados(totalCustodiados).emConformidade(custodiadosComp)
                    .inadimplentes(inadimplentes).taxaConformidade(taxaConf)
                    .taxaInadimplencia(100.0 - taxaConf).totalComparecimentos((long) comp.size()).build());
        }
    ...
    private ProximosComparecimentos calcularProximosComparecimentos(int dias) {
        LocalDate hoje = LocalDate.now();
        List<Custodiado> custodiados = custodiadoRepository.findByProximoComparecimentoBetween(hoje, hoje.plusDays(dias));
        Map<LocalDate, List<Custodiado>> porDia = new TreeMap<>();
        custodiados.forEach(c -> porDia.computeIfAbsent(c.getProximoComparecimento(), k -> new ArrayList<>()).add(c));

        List<ComparecimentoDiario> detalhes = porDia.entrySet().stream().map(e -> ComparecimentoDiario.builder()
                .data(e.getKey()).diaSemana(e.getKey().format(DateTimeFormatter.ofPattern("EEEE", new Locale("pt", "BR"))))
                .totalPrevisto(e.getValue().size())
                .custodiados(e.getValue().stream().map(c -> DetalheCustodiado.builder()
                        .id(c.getId()).nome(c.getNome()).processo(c.getProcesso())
                        .periodicidade(c.getPeriodicidadeDescricao()).diasAtraso(c.getDiasAtraso()).build())
                        .collect(Collectors.toList())).build()).collect(Collectors.toList());

        List<Custodiado> atrasados = custodiadoRepository.findByProximoComparecimentoBefore(hoje);

        return ProximosComparecimentos.builder().diasAnalisados(dias)
                .totalPrevistoProximosDias(custodiados.size()).totalAtrasados(atrasados.size())
                ...
                .custodiadosAtrasados(atrasados.stream().limit(10).map(c -> DetalheCustodiado.builder()
                        .id(c.getId()).nome(c.getNome()).processo(c.getProcesso())
                        .periodicidade(c.getPeriodicidadeDescricao()).diasAtraso(c.getDiasAtraso()).build())
    ...
    private AnaliseComparecimentos calcularAnaliseComparecimentos() {
        LocalDate hoje = LocalDate.now();
        List<HistoricoComparecimento> ultimos = historicoComparecimentoRepository.findByDataComparecimentoBetween(hoje.minusDays(30), hoje);
        long total = ultimos.size();
        long online = ultimos.stream().filter(h -> h.getTipoValidacao() == TipoValidacao.ONLINE).count();
        long presenciais = ultimos.stream().filter(h -> h.getTipoValidacao() == TipoValidacao.PRESENCIAL).count();

        return AnaliseComparecimentos.builder().comparecimentosUltimos30Dias(total)
                .comparecimentosOnlineUltimos30Dias(online).comparecimentosPresenciaisUltimos30Dias(presenciais)
                .taxaOnlineUltimos30Dias(total > 0 ? (double) online / total * 100 : 0.0)
                .comparecimentosPorDiaSemana(ultimos.stream().collect(Collectors.groupingBy(
                        h -> h.getDataComparecimento().getDayOfWeek().getDisplayName(java.time.format.TextStyle.FULL, Locale.forLanguageTag("pt-BR")), Collectors.counting())))
                .comparecimentosPorHora(ultimos.stream().filter(h -> h.getHoraComparecimento() != null)
                        .collect(Collectors.groupingBy(h -> h.getHoraComparecimento().getHour(), Collectors.counting()))).build();
    }
    ...
    private AnaliseAtrasos calcularAnaliseAtrasos() {
        ...
        distribuicao.put("1-7 dias", 0L); distribuicao.put("8-15 dias", 0L); distribuicao.put("16-30 dias", 0L);
        distribuicao.put("31-60 dias", 0L); distribuicao.put("61-90 dias", 0L); distribuicao.put("Mais de 90 dias", 0L);

        DetalheCustodiadoAtrasado maiorAtraso = null;
        long maiorDias = 0, soma = 0, count = 0;
        ...
        for (Custodiado c : inadimplentes) {
            if (c.getProximoComparecimento() != null && c.getProximoComparecimento().isBefore(hoje)) {
                ...
                try { if (c.getHistoricoEnderecos() != null && !c.getHistoricoEnderecos().isEmpty()) endereco = c.getEnderecoResumido(); } catch (Exception ignored) {}

                DetalheCustodiadoAtrasado det = DetalheCustodiadoAtrasado.builder()
                        .id(c.getId()).nome(c.getNome()).processo(c.getProcesso()).periodicidade(c.getPeriodicidadeDescricao())
                        .diasAtraso(dias).dataUltimoComparecimento(c.getUltimoComparecimento())
                        .dataProximoComparecimento(c.getProximoComparecimento())
                        .vara(c.getVara()).comarca(c.getComarca()).contato(c.getContato()).enderecoAtual(endereco).build();

                if (dias > maiorDias) { maiorDias = dias; maiorAtraso = det; }
                ...
                else if (dias <= 60) { distribuicao.merge("31-60 dias", 1L, Long::sum); t30++; }
                else if (dias <= 90) { distribuicao.merge("61-90 dias", 1L, Long::sum); t60++; }
                else { distribuicao.merge("Mais de 90 dias", 1L, Long::sum); tMais90++; }
                if (dias > 90) t90++;
            }
        }

        return AnaliseAtrasos.builder().totalCustodiadosAtrasados(inadimplentes.size())
                .totalAtrasados30Dias(t30).totalAtrasados60Dias(t60).totalAtrasados90Dias(t90).totalAtrasadosMais90Dias(tMais90)
                .mediaDiasAtraso(count > 0 ? (double) soma / count : 0.0)
                .distribuicaoAtrasos(distribuicao).custodiadoMaiorAtraso(maiorAtraso).dataAnalise(hoje).build();
    }
    ...
    public static class ResumoSistema {
        ...
        private LocalDate dataConsulta; private RelatorioUltimosMeses relatorioUltimosMeses;
        private List<TendenciaMensal> tendenciaConformidade; private ProximosComparecimentos proximosComparecimentos;
        private AnaliseComparecimentos analiseComparecimentos; private AnaliseAtrasos analiseAtrasos;
    }
    ...
    public static class TendenciaMensal {
        private String mes; private String mesNome; private long totalCustodiados; private long emConformidade;
        private long inadimplentes; private double taxaConformidade; private double taxaInadimplencia; private long totalComparecimentos;
    }
    ...
    public static class ProximosComparecimentos {
        ...
        private List<ComparecimentoDiario> detalhesPorDia; private List<DetalheCustodiado> custodiadosAtrasados;
    }

    @lombok.Data @lombok.Builder
    public static class ComparecimentoDiario {
        private LocalDate data; private String diaSemana; private int totalPrevisto; private List<DetalheCustodiado> custodiados;
    }

    @lombok.Data @lombok.Builder
    public static class DetalheCustodiado {
        private Long id; private String nome; private String processo; private String periodicidade; private long diasAtraso;
    }

    @lombok.Data @lombok.Builder
    public static class AnaliseComparecimentos {
        private long comparecimentosUltimos30Dias; private long comparecimentosOnlineUltimos30Dias;
        private long comparecimentosPresenciaisUltimos30Dias; private double taxaOnlineUltimos30Dias;
        private Map<String, Long> comparecimentosPorDiaSemana; private Map<Integer, Long> comparecimentosPorHora;
    }
    ...
    public static class AnaliseAtrasos {
        ...
        private DetalheCustodiadoAtrasado custodiadoMaiorAtraso; private LocalDate dataAnalise;
    }

    @lombok.Data @lombok.Builder
    public static class DetalheCustodiadoAtrasado {
        private Long id; private String nome; private String processo; private String periodicidade; private long diasAtraso;
        private LocalDate dataUltimoComparecimento; private LocalDate dataProximoComparecimento;
        private String vara; private String comarca; private String contato; private String enderecoAtual;
    }
}
